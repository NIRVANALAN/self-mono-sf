...
from utils.sceneflow_util import (
    pixel2pts_ms,
    pts2pixel_ms,
    reconstructImg,
    reconstructPts,
    projectSceneFlow2Flow,
)
...
def _adaptive_disocc_detection_disp(disp):
    ...
    mask = (
        torch.ones(b, 1, h, w, dtype=disp.dtype, device=disp.device)
        .float()
        .requires_grad_(False)
    )
    flow = (
        torch.zeros(b, 2, h, w, dtype=disp.dtype, device=disp.device)
        .float()
        .requires_grad_(False)
    )
    flow[:, 0:1, :, :] = disp * w
    ...
    if disocc_map.float().sum() < (b * h * w / 2):
        disocc_map = torch.ones(
            b, 1, h, w, dtype=torch.bool, device=disp.device
        ).requires_grad_(False)
...
def _disp2depth_kitti_K(disp, k_value):
    ...
    mask = (disp > 0).float()
    depth = (
        k_value.unsqueeze(1).unsqueeze(1).unsqueeze(1) * 0.54 / (disp + (1.0 - mask))
    )
...
class Loss_SceneFlow_SelfSup(nn.Module):
    ...
    def depth_loss_left_img(self, disp_l, disp_r, img_l_aug, img_r_aug, ii):
        ...
        # Photometric loss
        img_diff = (
            _elementwise_l1(img_l_aug, img_r_warp) * (1.0 - self._ssim_w)
            + _SSIM(img_l_aug, img_r_warp) * self._ssim_w
    ...
    def sceneflow_loss(
        ...
        img_diff1 = (
            _elementwise_l1(img_l1_aug, img_l2_warp) * (1.0 - self._ssim_w)
            + _SSIM(img_l1_aug, img_l2_warp) * self._ssim_w
        ).mean(dim=1, keepdim=True)
        img_diff2 = (
            _elementwise_l1(img_l2_aug, img_l1_warp) * (1.0 - self._ssim_w)
            + _SSIM(img_l2_aug, img_l1_warp) * self._ssim_w
        ).mean(dim=1, keepdim=True)
        ...
        # Loss Summnation
        sceneflow_loss = (
            loss_im + self._sf_3d_pts * loss_pts + self._sf_3d_sm * loss_3d_s
        )
    ...
    def forward(self, output_dict, target_dict):
        ...
        for ii, (sf_f, sf_b, disp_l1, disp_l2, disp_r1, disp_r2) in enumerate(
            ...
            loss_dp_sum = (
                loss_dp_sum + (loss_disp_l1 + loss_disp_l2) * self._weights[ii]
            )

            # Sceneflow Loss
            loss_sceneflow, loss_im, loss_pts, loss_3d_s = self.sceneflow_loss(
                sf_f,
                sf_b,
                disp_l1,
                disp_l2,
...
class Loss_SceneFlow_SemiSupFinetune(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        for ii, sf_f in enumerate(output_dict["flow_f_pp"]):
            ...
            disp_l1 = (
                interpolate2d_as(
                    output_dict["disp_l1_pp"][ii], gt_disp1, mode="bilinear"
                )
                * w_dp
            )
...
def eval_module_disp_depth(gt_disp, gt_disp_mask, output_disp, gt_depth, output_depth):
    ...
    d_outlier_epe = (
        (d_valid_epe > 3).float()
        * ((d_valid_epe / gt_disp) > 0.05).float()
        * gt_disp_mask_f
    )
    ...
    abs_rel, sq_rel, rms, log_rms, a1, a2, a3 = compute_errors(
        gt_depth[gt_disp_mask], output_depth[gt_disp_mask]
    )
...
class Eval_MonoDepth_Eigen(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        out_disp_l1 = interpolate2d_as(
            output_dict["disp_l1_pp"][0], gt_depth, mode="bilinear"
        ) * gt_depth.size(3)
        out_depth_l1 = _disp2depth_kitti_K(
...
class Eval_SceneFlow_KITTI_Test(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        out_disp_l1 = interpolate2d_as(
            output_dict["disp_l1_pp"][0], input_l1, mode="bilinear"
        ) * input_l1.size(3)
...
class Eval_SceneFlow_KITTI_Train(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        flow_gt_mag = (
            torch.norm(target_dict["target_flow"], p=2, dim=1, keepdim=True) + 1e-8
        )
        flow_outlier_epe = (
            (valid_epe > 3).float()
            * ((valid_epe / flow_gt_mag) > 0.05).float()
            * gt_flow_mask
        )
        loss_dict["f1"] = (
            flow_outlier_epe.view(batch_size, -1).sum(1)
        ).mean() / 91875.68
        ...
        dict_disp1_occ = eval_module_disp_depth(
            gt_disp2_occ,
            gt_disp2_mask.bool(),
            out_disp_l1_next,
            gt_depth_l1_next,
        ...
        outlier_sf = (
            flow_outlier_epe.bool() + d0_outlier_image.bool() + d1_outlier_image.bool()
        ).float() * gt_sf_mask
...
class Loss_SceneFlow_SelfSup_NoOcc(nn.Module):
    ...
    def sceneflow_loss(
        ...
        _, pts1_tf, coord1_sf_f = pts2pixel_ms(k1_scale, pts1, sf_f, [h_dp, w_dp])
        _, pts2_tf, coord2_sf_b = pts2pixel_ms(k2_scale, pts2, sf_b, [h_dp, w_dp])

        pts2_warp = reconstructPts(coord1_sf_f, pts2)
        pts1_warp = reconstructPts(coord2_sf_b, pts1)
        ...
        img_l2_warp = reconstructImg(coord1_sf_f, img_l2_aug)
        img_l1_warp = reconstructImg(coord2_sf_b, img_l1_aug)
...
class Loss_SceneFlow_SelfSup_NoPts(nn.Module):
    ...
    def sceneflow_loss(
        ...
        # Loss Summnation
        sceneflow_loss = (
            loss_im + self._sf_3d_sm * loss_3d_s
        )  # + self._sf_3d_pts * loss_pts
    ...
    def forward(self, output_dict, target_dict):
        ...
        for ii, (sf_f, sf_b, disp_l1, disp_l2, disp_r1, disp_r2) in enumerate(
            ...
            # Sceneflow Loss
            loss_sceneflow, loss_im, loss_3d_s = self.sceneflow_loss(
                sf_f,
                sf_b,
                disp_l1,
                disp_l2,
...
class Loss_Flow_Only(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        for ii, (sf_f, sf_b) in enumerate(
            zip(output_dict["flow_f"], output_dict["flow_b"])
        ):
            ...
            img_diff1 = (
                _elementwise_l1(img_l1, img_l2_warp) * (1.0 - self._ssim_w)
                + _SSIM(img_l1, img_l2_warp) * self._ssim_w
            ).mean(dim=1, keepdim=True)
            img_diff2 = (
                _elementwise_l1(img_l2, img_l1_warp) * (1.0 - self._ssim_w)
                + _SSIM(img_l2, img_l1_warp) * self._ssim_w
            ).mean(dim=1, keepdim=True)
            loss_im1 = img_diff1[occ_map_f].mean()
            loss_im2 = img_diff2[occ_map_b].mean()
            img_diff1[~occ_map_f].detach_()
            ...
            loss_smooth = (
                _smoothness_motion_2nd(sf_f / 20.0, img_l1, beta=10.0).mean()
...
class Loss_Disp_Only(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        for ii, (disp_l1, disp_l2, disp_r1, disp_r2) in enumerate(
            zip(
                output_dict["disp_l1"],
                output_dict["disp_l2"],
                disp_r1_dict,
...
class Eval_Disp_Only(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        out_disp_l1 = interpolate2d_as(
            output_dict["disp_l1_pp"][0], gt_disp, mode="bilinear"
        ) * gt_disp.size(3)
        out_depth_l1 = _disp2depth_kitti_K(out_disp_l1, intrinsics[:, 0, 0])
        out_depth_l1 = torch.clamp(out_depth_l1, 1e-3, 80)
...
class Basis_MonoDepthLoss(nn.Module):
    ...
    def forward(self, disp_l, disp_r, img_l, img_r):
        ...
        # Image reconstruction loss
        left_est = [
            self.generate_image_left(img_r_pyramid[i], disp_l[i]) for i in range(self.n)
        ]
        right_est = [
            self.generate_image_right(img_l_pyramid[i], disp_r[i])
            for i in range(self.n)
        ]
        ...
        l1_left = [
            torch.mean(
                (torch.abs(left_est[i] - img_l_pyramid[i])).mean(dim=1, keepdim=True)[
                    left_occ[i]
                ]
            )
            for i in range(self.n)
        ...
        l1_right = [
            torch.mean(
                (torch.abs(right_est[i] - img_r_pyramid[i])).mean(dim=1, keepdim=True)[
                    right_occ[i]
                ]
            )
            for i in range(self.n)
        ...
        ssim_left = [
            torch.mean(
                (self.SSIM(left_est[i], img_l_pyramid[i])).mean(dim=1, keepdim=True)[
                    left_occ[i]
                ]
            )
            for i in range(self.n)
        ...
        ssim_right = [
            torch.mean(
                (self.SSIM(right_est[i], img_r_pyramid[i])).mean(dim=1, keepdim=True)[
                    right_occ[i]
                ]
            )
            for i in range(self.n)
        ...
        right_left_disp = [
            self.generate_image_left(disp_r[i], disp_l[i]) for i in range(self.n)
        ]
        left_right_disp = [
            self.generate_image_right(disp_l[i], disp_r[i]) for i in range(self.n)
        ]

        lr_left_loss = [
            torch.mean((torch.abs(right_left_disp[i] - disp_l[i]))[left_occ[i]])
            for i in range(self.n)
        ]
        lr_right_loss = [
            torch.mean((torch.abs(left_right_disp[i] - disp_r[i]))[right_occ[i]])
            for i in range(self.n)
        ]
...
class Loss_MonoDepth(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        depth_loss = self._depth_loss(
            output_dict["disp_l1"],
            output_dict["disp_r1"],
            target_dict["input_l1"],
            target_dict["input_r1"],
        )
