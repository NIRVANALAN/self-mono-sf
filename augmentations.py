...
class PhotometricAugmentation(nn.Module):
    ...
    def forward(self, *args):
        ...
        rand_gamma = (
            torch.rand(
                [b, 1, 1, 1], dtype=d_dtype, device=d_device, requires_grad=False
            )
            * self._intv_gamma
            + self._min_gamma
        )
        ...
        rand_shift = (
            torch.rand(
                [b, 3, 1, 1], dtype=d_dtype, device=d_device, requires_grad=False
            )
            * self._intv_shift
            + self._min_shift
        )
...
class Augmentation_ScaleCrop(nn.Module):
    def __init__(
        self, args, photometric=True, trans=0.07, scale=[0.93, 1.0], resize=[256, 832]
    ):
    ...
    def find_invalid(self, img_size, params):
        ...
        pt_mat = torch.cat(
            [
                torch.cat([hf_w, hf_o, hf_o], dim=2),
                torch.cat([hf_o, hf_h, hf_o], dim=2),
                torch.cat([hf_o, hf_o, hf_i], dim=2),
            ],
            dim=1,
        )
    ...
    def calculate_tform_and_grids(self, img_size, resize, params):
        ...
        grid_pts = (
            torch.cat([grid_ww, grid_hh, torch.ones_like(grid_hh)], dim=0)
            .unsqueeze(0)
            .expand(self._batch, -1, -1, -1)
        )
        ...
        tr_tform[:, 0, 2] = tx[:, 0]
        tr_tform[:, 1, 2] = ty[:, 0]
        pts_tform = torch.matmul(tr_tform, pts_tform).view(
            self._batch, 3, resize[0], resize[1]
        )

        grid_img_ww = (
            pts_tform[:, 0, :, :] / float(img_size[1]) * 2
        )  # x2 is for scaling [-1. 1]
        grid_img_hh = pts_tform[:, 1, :, :] / float(img_size[0]) * 2

        grid_img = torch.cat(
            [grid_img_ww.unsqueeze(3), grid_img_hh.unsqueeze(3)], dim=3
        )
    ...
    def find_aug_params(self, img_size, resize):
        ...
        while invalid.sum() > 0:
            ...
            max_t = (
                torch.min(torch.ones_like(scale) - scale, max_trans) * 0.5
            )  # 0.5 because the translation rage is [-0.5*trans, 0.5*trans]
    ...
    def augment_intrinsic_matrices(
        self, intrinsics, num_splits, img_size, resize, params
    ):
        ...
        str_p_ww = (
            pts_tform[:, 0, :]
            + torch.ones_like(pts_tform[:, 0, :]) * float(img_size[1]) * 0.5
        )
        str_p_hh = (
            pts_tform[:, 1, :]
            + torch.ones_like(pts_tform[:, 1, :]) * float(img_size[0]) * 0.5
        )
...
class Augmentation_SceneFlow(Augmentation_ScaleCrop):
    def __init__(
        ...
        super(Augmentation_SceneFlow, self).__init__(
            args, photometric=photometric, trans=trans, scale=scale, resize=resize
        )
...
class Augmentation_MonoDepth(Augmentation_ScaleCrop):
    def __init__(
        self, args, photometric=True, trans=0.1, scale=[0.9, 1.0], resize=[256, 512]
    ):
...
class Augmentation_SceneFlow_Finetuning(nn.Module):
    ...
    def kitti_random_crop(self, example_dict):
        ...
        ## Cropping
        example_dict["input_l1"] = self.cropping(
            example_dict["input_l1"], str_x, str_y, end_x, end_y
        )
        example_dict["input_l2"] = self.cropping(
            example_dict["input_l2"], str_x, str_y, end_x, end_y
        )
        example_dict["input_r1"] = self.cropping(
            example_dict["input_r1"], str_x, str_y, end_x, end_y
        )
        example_dict["input_r2"] = self.cropping(
            example_dict["input_r2"], str_x, str_y, end_x, end_y
        )

        example_dict["target_flow"] = self.cropping(
            example_dict["target_flow"], str_x, str_y, end_x, end_y
        )
        example_dict["target_flow_mask"] = self.cropping(
            example_dict["target_flow_mask"], str_x, str_y, end_x, end_y
        )
        example_dict["target_flow_noc"] = self.cropping(
            example_dict["target_flow_noc"], str_x, str_y, end_x, end_y
        )
        example_dict["target_flow_mask_noc"] = self.cropping(
            example_dict["target_flow_mask_noc"], str_x, str_y, end_x, end_y
        )

        example_dict["target_disp"] = self.cropping(
            example_dict["target_disp"], str_x, str_y, end_x, end_y
        )
        example_dict["target_disp_mask"] = self.cropping(
            example_dict["target_disp_mask"], str_x, str_y, end_x, end_y
        )
        example_dict["target_disp2_occ"] = self.cropping(
            example_dict["target_disp2_occ"], str_x, str_y, end_x, end_y
        )
        example_dict["target_disp2_mask_occ"] = self.cropping(
            example_dict["target_disp2_mask_occ"], str_x, str_y, end_x, end_y
        )

        example_dict["target_disp_noc"] = self.cropping(
            example_dict["target_disp_noc"], str_x, str_y, end_x, end_y
        )
        example_dict["target_disp_mask_noc"] = self.cropping(
            example_dict["target_disp_mask_noc"], str_x, str_y, end_x, end_y
        )
        example_dict["target_disp2_noc"] = self.cropping(
            example_dict["target_disp2_noc"], str_x, str_y, end_x, end_y
        )
        example_dict["target_disp2_mask_noc"] = self.cropping(
            example_dict["target_disp2_mask_noc"], str_x, str_y, end_x, end_y
        )

        example_dict["input_k_l1"] = _intrinsic_crop(
            example_dict["input_k_l1"], str_x, str_y
        )
        example_dict["input_k_l2"] = _intrinsic_crop(
            example_dict["input_k_l2"], str_x, str_y
        )
        example_dict["input_k_r1"] = _intrinsic_crop(
            example_dict["input_k_r1"], str_x, str_y
        )
        example_dict["input_k_r2"] = _intrinsic_crop(
            example_dict["input_k_r2"], str_x, str_y
        )
...
class Augmentation_Resize_Only(nn.Module):
    ...
    def forward(self, example_dict):
        ...
        if (
            self._photometric and torch.rand(1) > 0.5
        ):  # * flip X, change Px focal center in K
            if self._isRight:
                im_l1, im_l2, im_r1, im_r2 = self._photo_augmentation(
                    im_l1, im_l2, im_r1, im_r2
                )
