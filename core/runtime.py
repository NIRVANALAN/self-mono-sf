...
def create_progressbar(
    ...
    tqdm_args = {
        ...
        "miniters": 1
        if train
        else None,  # Minimum display update interval in iterations
...
def format_moving_averages_as_progress_dict(
    moving_averages_dict={}, moving_averages_postfix="avg"
):
    ...
    def run(self, model_and_loss, optimizer):
        ...
        with create_progressbar(**progressbar_args) as progress:
            for example_dict in progress:
                ...
                # perform step
                loss_dict_per_step, output_dict = self._step(
                    example_dict, model_and_loss, optimizer
                )
                # convert
                ...
                for key, loss in loss_dict_per_step.items():
                    moving_averages_dict[key].add_average(
                        loss, addcount=self._args.batch_size
                    )
...
class EvaluationEpoch:
    ...
    def save_outputs(self, example_dict, output_dict):
        ...
        if self._args.save_flow:
            ...
            for ii in range(0, b_size):
                file_name_flow = (
                    save_root_flow + "/" + str(example_dict["basename"][ii])
                )
        ...
        if self._args.save_disp:
            ...
            for ii in range(0, b_size):
                file_name_disp = (
                    save_root_disp + "/" + str(example_dict["basename"][ii])
                )
        ...
        if self._args.save_disp2:
            ...
            for ii in range(0, b_size):
                file_name_disp2 = (
                    save_root_disp2 + "/" + str(example_dict["basename"][ii])
                )
            ...
            for ii in range(0, b_size):
                ...
                plt.imsave(
                    file_names_disp2[ii] + "_disp2.jpg", norm_disp2, cmap="plasma"
                )
                # Png
    ...
    def _step(self, example_dict, model_and_loss):
        ...
        # Optionally perform augmentations
        if self._augmentation is not None:
            example_dict = self._augmentation(example_dict)  # * resize only here
...
def exec_runtime(
    ...
    best_validation_loss = (
        float("inf") if args.validation_key_minimize else -float("inf")
    )
    store_as_best = False
    ...
    for epoch in range(args.start_epoch, args.total_epochs + 1):
        with logger.LoggingBlock("Epoch %i/%i" % (epoch, args.total_epochs), emph=True):
            ...
            # -------------------------------------------
            if train_loader is not None:
                avg_loss_dict, _ = training_module.run(
                    model_and_loss=model_and_loss, optimizer=optimizer
                )

                if args.evaluation is False:
                    tensorBoardWriter.add_scalar(
                        "Train/Loss", avg_loss_dict[args.training_key], epoch
                    )
            ...
            if validation_loader is not None:
                ...
                # ---------------------------------------------------
                avg_loss_dict, output_dict = evaluation_module.run(
                ...
                if args.evaluation is False:
                    tensorBoardWriter.add_scalar(
                        "Val/Metric", avg_loss_dict[args.validation_key], epoch
                    )
