...
class MonoSceneFlow(nn.Module):
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            if l == 0:
                ...
                x2_warp = self.warping_layer_sf(
                    x2, flow_f, disp_l1, k1, input_dict["aug_size"]  # TODO
            ...
            # monosf estimator
            if l == 0:
                x1_out, flow_f, disp_l1 = self.flow_estimators[l](
                    torch.cat([out_corr_relu_f, x1], dim=1)
                )
                x2_out, flow_b, disp_l2 = self.flow_estimators[l](
    ...
    def forward(self, input_dict):
        ...
        if self._args.evaluation or self._args.finetuning:
            ...
            )  # ? sends flipped input (K, I) into pwc_net?
            ...
            for ii in range(0, len(output_dict_flip["flow_f"])):
                ...
                flow_f_pp.append(
                    post_processing(
                        output_dict["flow_f"][ii],
                        flow_horizontal_flip(
                            output_dict_flip["flow_f"][ii]
                        ),  #! flip back
                    )
                )  # ?
                flow_b_pp.append(
                    post_processing(
                        output_dict["flow_b"][ii],
                        flow_horizontal_flip(output_dict_flip["flow_b"][ii]),
                    )
                )
                disp_l1_pp.append(
                    post_processing(
                        output_dict["disp_l1"][ii],
                        torch.flip(output_dict_flip["disp_l1"][ii], [3]),
                    )
                )
                disp_l2_pp.append(
                    post_processing(
                        output_dict["disp_l2"][ii],
                        torch.flip(output_dict_flip["disp_l2"][ii], [3]),
                    )
                )
